...
def info_project(message, user_id, project_name):
    ...
    if not info:
        bot.send_message(message.chat.id, "Информация о проекте не найдена!")
        return

    info = info[0]  # теперь info — это кортеж: (project_name, description, url, status_name)
...
/start 🎉😀- забускает бота 
/info ⚙️ - посмотреть команды
/new_project ➕- используй для добавления нового проекта 
/skills 📒- используй для добавления скилов
/projects 📒🤖- используй для просмотра проектов
/delete ❌- используй для удаления проекта
/update_projects 🔄- используй для обновления проекта

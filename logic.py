...
class DB_Manager:
    ...
    def insert_skill(self, user_id, project_name, skill):
        ...
        skill_id = self.__select_data('SELECT id FROM skills WHERE skill_name = ?', (skill,))[0][0]
...
JOIN skills ON skills.id = project_skills.id 
...
"""
    ...
    def add_photo_column(self):
        conn = sqlite3.connect(self.database)
        with conn:
            conn.execute("ALTER TABLE projects ADD COLUMN photo TEXT AFTER st")
            conn.commit()

    def update_photo_for_project(self, user_id, project_name, photo_path):
        sql = "UPDATE projects SET photo = ? WHERE user_id = ? AND project_name = ?"
        self.__executemany(sql, [(photo_path, user_id, project_name)])
